...
const createProject = async (project, otherArg) => {
  // 1.提示信息
  log.hint('guoshang helps you create your project, please wait a moment~');
  const gitrepoer = await gitrepo();
  console.log('download templete...');
  // 2.clone项目从仓库
  await downloadRepo(gitrepoer, project, { clone: true });
  console.log('download templete is over');
  console.log('auto run npm install...');
  // 3.执行终端命令npm install
