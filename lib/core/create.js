...
const createCommands = () => {
  ...
  program
    ...
    .description(
      'add vue component, 例如: guoshang addcpn NavBar [-d src/components]'
    )
    .action((name) => addComponent(name, program.dest || 'src/components'));

  program
    .command('addpage <name>')
    .description('add vue page, 例如: guoshang addpage Home [-d dest]')
    .action((name) => {
  ...
  program
    ...
    .description('add vue store, 例如: guoshang addstore favor [-d dest]')
    .action((name) => {
